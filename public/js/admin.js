      ...
      <div class="room-content">
        <div class="room-header">
          ...
          <div class="action-btns">
            <button class="btn btn-sm" onclick="viewRoom('${room.id}')" title="Просмотр">👁️</button>
...
function viewRoom(id) {
  ...
  const modal = document.getElementById('roomViewModal') || createRoomViewModal();
  const title = modal.querySelector('#roomViewModalTitle');
  const body = modal.querySelector('#roomViewModalBody');
  
  title.textContent = room.name;
  
  // Build images gallery
  const images = room.images?.length > 0 ? room.images : ['https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=600&q=80'];
  const imagesHtml = images.map((img, idx) => 
    `<img src="${img}" alt="${room.name}" class="room-gallery-img" style="width: 100%; height: 250px; object-fit: cover; border-radius: 8px; ${idx > 0 ? 'display: none;' : ''}" data-idx="${idx}" />`
  ).join('');
  ...
  body.innerHTML = `
    <div class="room-gallery" style="position: relative; margin-bottom: 20px;">
      ${imagesHtml}
      ${images.length > 1 ? `
        <button onclick="adminPrevRoomImage()" style="position: absolute; left: 10px; top: 50%; transform: translateY(-50%); background: rgba(0,0,0,0.7); color: white; border: none; padding: 10px; border-radius: 50%; cursor: pointer;">❮</button>
        <button onclick="adminNextRoomImage()" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: rgba(0,0,0,0.7); color: white; border: none; padding: 10px; border-radius: 50%; cursor: pointer;">❯</button>
        <div style="text-align: center; margin-top: 10px;">
          ${images.map((_, idx) => `<span onclick="adminShowRoomImage(${idx})" style="display: inline-block; width: 10px; height: 10px; border-radius: 50%; background: ${idx === 0 ? '#0EA5E9' : '#ccc'}; margin: 0 5px; cursor: pointer;"></span>`).join('')}
        </div>
      ` : ''}
    </div>
    ...
    <div class="room-info">
      ...
      <p style="font-size: 14px; line-height: 1.6; margin-bottom: 20px; color: var(--text-secondary);">${room.description || 'Описание отсутствует'}</p>
      
      <h4 style="margin-bottom: 10px;">Удобства:</h4>
      <div style="flex-wrap: wrap; gap: 8px;">
        ${featuresHtml || '<span style="color: var(--gray);">Нет информации об удобствах</span>'}
      </div>
      
      <div style="margin-top: 20px; padding: 20px; background: var(--bg-secondary); border-radius: 8px;">
        <span style="font-size: 28px; font-weight: bold; color: var(--primary);">${(room.price_from || 0).toLocaleString()} ₽</span>
...
let adminRoomImages = [];
let adminRoomImageIndex = 0;

function createRoomViewModal() {
  const modal = document.createElement('div');
  modal.id = 'roomViewModal';
  modal.className = 'modal';
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 700px;">
      <div class="modal-header">
        <h3 id="roomViewModalTitle">Номер</h3>
        <button class="modal-close" onclick="closeRoomViewModal()">&times;</button>
      </div>
      <div id="roomViewModalBody" style="padding: 20px;"></div>
      <div class="modal-actions">
        <button class="btn btn-secondary" onclick="closeRoomViewModal()">Закрыть</button>
      </div>
    </div>
  `;
  ...
  modal.addEventListener('click', (e) => {
    if (e.target === modal) closeRoomViewModal();
  });
  
  return modal;
}

function adminShowRoomImage(idx) {
  adminRoomImageIndex = idx;
  const images = document.querySelectorAll('#roomViewModalBody .room-gallery-img');
  const dots = document.querySelectorAll('#roomViewModalBody span[onclick^="adminShowRoomImage"]');
...
function adminPrevRoomImage() {
  const modal = document.getElementById('roomViewModal');
  const roomId = modal?.dataset?.roomId;
  if (!roomId) return;
  
  const room = data.hotel?.rooms?.find(r => r.id === roomId);
  const total = room?.images?.length || 1;
  adminShowRoomImage((adminRoomImageIndex - 1 + total) % total);
}

function adminNextRoomImage() {
  const modal = document.getElementById('roomViewModal');
  const roomId = modal?.dataset?.roomId;
  if (!roomId) return;
  ...
  const room = data.hotel?.rooms?.find(r => r.id === roomId);
  const total = room?.images?.length || 1;
  adminShowRoomImage((adminRoomImageIndex + 1) % total);
}
...
window.closeRoomViewModal = closeRoomViewModal;
window.adminShowRoomImage = adminShowRoomImage;
window.adminPrevRoomImage = adminPrevRoomImage;
window.adminNextRoomImage = adminNextRoomImage;
